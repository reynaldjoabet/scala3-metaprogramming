...
/** A parameterised SQL statement: the text never contains user data, only `?`
  * placeholders, and the values travel beside it.
  */
...
/** A checked SQL interpolator.
  ...
  *   - refuses any interpolated value that has no `SqlEncoder`, so a value can
  *     never be pasted into the statement text,
  *   - rejects `... where name = '$name'` - quoting a bound parameter is the
  *     classic way to turn a safe query into a broken (or injectable) one,
  *   - rejects statements with unbalanced quotes or parentheses. Note this is
  *     checked per interpolation, so each fragment has to balance on its own:
  *     build a parenthesised group inside one `sql"..."`, not by gluing
  *     `sql"("` to `sql")"`,
  *   - splices nested `SqlFragment`s so queries compose,
  *   - folds the whole statement into a *single constant string* in the
  *     bytecode when no nested fragment is involved: there is no runtime
...
/** Statement text derived from a case class at compile time.
  *
  * Every method here returns a `String` *constant*: the column list is computed
  * during compilation and lands in the constant pool, so there is no startup
  * cost and no chance of the statement drifting from the model - add a field
  * and the generated `insert` grows with it.
  */
object Table {

  /** Field names, snake_cased: `case class User(userName: String)` gives
    * `List("user_name")`.
    */
