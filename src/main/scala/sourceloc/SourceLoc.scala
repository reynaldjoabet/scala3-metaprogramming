...
/** Where a call came from, resolved at compile time.
  *
  * Every logging library needs this: a log line wants the file and line of the
  * *caller*, and the only two ways to get it are a stack trace (expensive, and
  * wrong under inlining) or a macro (free). `SourceLoc.here` is a `given`, so a
  * method can simply ask for one:
  *
  * {{{
  ...
  * }}}
  *
  * Each call site expands to a `SourceLoc(...)` built from string and int
  * constants - there is nothing to compute at runtime.
  *
  * @see
...
object Trace {
  ...
  /** Print an expression's *source text* together with its value, then return
    * the value - the trick behind ScalaTest/munit style assertion messages.
    *
    * {{{
