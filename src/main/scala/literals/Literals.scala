...
/**
  ...
  * Two separate wins:
  *
  *   - '''validation moves left'''. A malformed UUID or regex is a compile error pointing at the
  *     exact character that broke, not a `PatternSyntaxException` thrown the first time that code
  *     path runs in production.
  *   - '''parsing disappears'''. `uuid"..."` does not expand to `UUID.fromString("...")`; it
  *     expands to `new UUID(hi, lo)` with both halves already computed, so the runtime cost is one
  *     allocation and the string never even reaches the class file.
  *
  * @see
